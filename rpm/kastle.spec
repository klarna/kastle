...
erl -noshell -eval '{ok, [Config0]} = file:consult("rel/sys.config"), LagerConfig0 = proplists:get_value(lager, Config0, []), LagerConfig1 = lists:keystore(log_root, 1, LagerConfig0, {log_root, "%{_log_dir}"}), LagerConfig = lists:keystore(handlers, 1, LagerConfig1, {handlers, [{lager_console_backend,error},{lager_file_backend,[{file,"error.log"},{level,error}]}]}), Config1 = lists:keystore(lager, 1, Config0, {lager, LagerConfig}), KastleConfig0 = proplists:get_value(kastle, Config1, []), KastleConfig = lists:keystore(listeners, 1, KastleConfig0, {listeners, 64}), Config = lists:keystore(kastle, 1, Config1, {kastle, KastleConfig}), file:write_file("rel/sys.config", io_lib:format("~p.~n", [Config])), {ok, _} = file:consult("rel/sys.config"), halt(0).'
